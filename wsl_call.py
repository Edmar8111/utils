...
class SubPro:
  def __init__(self, install=None, uninstall=None,id_service=None):
    ...
    try:
      ...
      sys.exit(1)
    return
  def remover_distro(self, distro_name):
    pacote_arg=f'wls --unregister {distro_name}'
    try:
      resultado=subprocess.run(pacote_arg,check=True,capture_output=True,text=True)
      ...
      sys.exit(1)
    return
  def listar_distros(self):
    pacote_arg=f'wls --list --verbose'
    try:
      resultado=subprocess.run(pacote_arg,check=True,capture_output=True,text=True)
      return f'DISTROS INSTALADAS: \n {resultado.stdout}'
    except subprocess.CalledProcessError as erro:
      print(f'ERRO AO TENTAR RETORNAR DISTROS -> {erro.returncode} \n{erro.stderr}')
      sys.exit(1)
    return
